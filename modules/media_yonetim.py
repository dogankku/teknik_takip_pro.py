...
def render(secilen_tarih: date):
    ...
    col_labels = {
        "Media_ID": "ID", "Parent_Tip": "Modül", "Parent_ID": "Kayıt ID",
        "Dosya_Adi": "Dosya", "Boyut": "Boyut", "Yukleme_Tarihi": "Tarih", "Yukleyen": "Yükleyen",
    }
    data_table(
        g[show_cols],
        [(c, col_labels.get(c, c)) for c in show_cols],
        id_cols=["Media_ID"] if "Media_ID" in show_cols else (),
        avatar_cols=["Yukleyen"] if "Yukleyen" in show_cols else (),
        max_text=40,
    )
